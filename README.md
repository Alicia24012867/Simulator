...
`RS`、`RBE`、`RCE` 与 `RDS` 已落实到 DC / transient 的器件 stamp：Diode `RS` 是由外部阳极到本征二极管阳极的串联电阻（按 `AREA` 缩放）；BJT `RBE` / `RCE` 分别是本征 B-E / C-E 并联电阻（按 `AREA` 缩放）；Level-1 MOS 的 `RDS` 是按 `W/L` 缩放的 D-S 线性并联电导。MOS3 的 `TOX`、`LD`、`XL`、`WD`、`XW`、`UO`、`VTO`、`KP`、`GAMMA`、`PHI`、`NSUB`、`ETA`、`DELTA`、`THETA`、`VMAX`、`KAPPA` 已参与 DC channel stamp；`RD`/`RS`（或 `RSH*NRD/NRS`）会建立内部 D′/S′ 节点，B-D′/B-S′ 结电流按 `JS*AD/AS` 或 `IS` 参与 DC stamp，结电容则以 `CJ`、`CJSW`、`PB`、`MJ`、`MJSW`、`FC` 的瞬态 companion stamp 接入。器件温度和 Meyer 栅电荷仍待实现。未知参数、非数值参数、非物理的负值以及非 `LEVEL=1` / `LEVEL=3` 的 MOSFET model 会在读取阶段明确报错。
...
- 二极管、BJT 和 MOSFET 仍是有限子集。MOS3 已实现 DC channel current、`RD`/`RS`、`RSH*NRD/NRS`、B-D′/B-S′ 结电流及其耗尽结电容；器件温度和 Meyer 栅电荷尚未实现。
